...
public class ECommerceEliminarProductoSteps
{
    ...
    public void GivenTheUserHasAddedProductsToTheCart()
    {
        ...
        //_productosCarritoPage.AgregarProductosCarrito(data.Arguments[2], "Productos");
        //_productosCarritoPage.EjecutarCapturaEvidenciaEliminarProducto(_driver);
        var data = ECommerceProductosCarrito.Data.DataAgregarProductosCarrito
                .GetJsonDataAgregarProductos()
                .First();

        // Obtiene la lista de productos desde TestCaseData
        var productos = (List<int>)data.Arguments[2];

        _productosCarritoPage.AgregarProductosCarrito(productos, "Productos");
